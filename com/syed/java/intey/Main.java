...
// so for food items apply 20% tax levied and
// for non food items 10% tax is levied
// so input should be product name followed by quantity and price
// of it so then calculate the total price with tax for all products which includes both food and non food items
class Product {
    private String name;
    ...
    public double calculatePriceWithTax() {
        ...
        if (foodItems.contains(name.toLowerCase())) {
            taxRate = 0.20; // 20% tax for food items
        } else {
            taxRate = 0.10; // 10% tax for non-food items
        }
...
public class Main {
    public static void main(String[] args) {
        ...
        for (Product product : products) {
            double priceWithTax = product.calculatePriceWithTax();
        ...
        System.out.printf("Total price including tax: %.2f%n", totalPrice);
    }
}
